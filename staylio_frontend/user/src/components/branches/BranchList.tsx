  ...
  if (isLoading) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {[1, 2, 3, 4, 5, 6].map((i) => (
          <div key={i} className="animate-pulse bg-white rounded-2xl border border-gray-100 overflow-hidden flex flex-col h-[360px]">
  ...
  if (!branches || branches.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 px-4 text-center bg-gray-50/50 rounded-2xl border border-dashed border-gray-200">
        ...
        <h3 className="text-lg font-bold text-gray-900 mb-1">Chưa có khách sạn nào</h3>
        <p className="text-gray-500 max-w-sm">Hiện tại chưa có chi nhánh khách sạn nào hoạt động tại khu vực này. Vui lòng quay lại sau hoặc tìm kiếm ở khu vực khác.</p>
      </div>
    );
  }
