...
export default function BranchCard({ branch }: BranchCardProps) {
  ...
  return (
    <div
      ...
      className="group bg-white rounded-2xl border border-gray-100 overflow-hidden cursor-pointer hover:shadow-xl hover:border-transparent hover:-translate-y-1 transition-all duration-300 flex flex-col"
    >
      <div className="relative h-48 sm:h-56 overflow-hidden">
        ...
        {/* Rating Badge */}
        <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm px-2.5 py-1.5 rounded-lg flex items-center gap-1.5 shadow-sm">
          <Star className="w-4 h-4 text-yellow-500 fill-yellow-500" />
          <span className="text-sm font-bold text-gray-900">{branch.averageRating > 0 ? branch.averageRating.toFixed(1) : "Chưa có"}</span>
          {branch.countReview > 0 && (
            <span className="text-xs text-gray-500 font-medium">({branch.countReview})</span>
          )}
        </div>
      ...
      <div className="p-5 flex flex-col grow">
        <h3 className="text-lg font-bold text-gray-900 line-clamp-1 group-hover:text-[#0066FF] transition-colors">
