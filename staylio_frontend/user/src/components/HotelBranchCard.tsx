    ...
    >
      <div className="relative h-52 w-full overflow-hidden">
        ...
        <div className="absolute bottom-3 right-3 bg-white/90 backdrop-blur-sm px-2 py-1 rounded-lg flex items-center gap-1 shadow-sm">
          ...
          <span className="text-xs font-bold text-gray-800">{branch.averageRating?.toFixed(1) || "0.0"}</span>
        </div>
      </div>
      ...
      <div className="p-4 flex flex-col grow">
        ...
        <div className="flex items-center gap-2 mb-3">
          <div className="flex">
            {[...Array(5)].map((_, i) => (
              <Star 
                key={i} 
                className={`w-3.5 h-3.5 ${i < Math.round(branch.averageRating || 0) ? 'fill-yellow-400 text-yellow-400' : 'text-gray-200'}`} 
              />
            ))}
          </div>
        ...
        {/* Thay tiện ích bằng tên thương hiệu khách sạn */}
        <div className="mb-4">
          <span className="text-xs font-semibold text-blue-700 bg-blue-50 px-2.5 py-1.5 rounded-lg border border-blue-100 line-clamp-1">
