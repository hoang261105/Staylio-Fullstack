    ...
    <div className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 flex flex-col h-full group">
      <div className="relative h-48 w-full overflow-hidden">
        ...
        <div className="absolute bottom-3 right-3 bg-white/90 backdrop-blur-sm px-2 py-1 rounded-lg flex items-center gap-1 shadow-sm">
          ...
          <span className="text-xs font-bold text-gray-800">{room.averageRating?.toFixed(1) || "0.0"}</span>
        </div>
      </div>
      ...
      <div className="p-5 flex flex-col grow">
        <div className="flex justify-between items-start mb-2">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1 uppercase font-medium tracking-wide">
              {room.roomType}
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-1.5 mb-5">
          {room.utilities?.slice(0, 3).map((utility) => {
            ...
            return (
              <div key={utility.id} className="flex items-center gap-2 text-gray-600">
        ...
        <div className="mt-auto pt-4 border-t border-gray-100 flex items-end justify-between">
          <div>
            <p className="text-[10px] text-gray-400 font-semibold uppercase tracking-wider mb-0.5">Giá mỗi đêm</p>
