...
import { useUpdateVoucherMutation } from "@common/hooks/useVouchers";
...
export default function VoucherFormUpdate({
  onClose,
  isOpen,
  voucher,
}: VoucherFormUpdateProps) {
  ...
  const { mutateAsync: updateVoucher, isPending } = useUpdateVoucherMutation(
    voucher?.id ?? 0,
  );
  ...
  useEffect(() => {
    if (isOpen && voucher) {
      setFormData({
        ...
        hotelBranchId: voucher.hotelBranchId,
      ...
      if (branches && branches.length > 0) {
        const branch = branches.find((b) => b.id === voucher.hotelBranchId);
        if (branch) {
          setSelectedBranch({
            value: branch.id,
          ...
          setSelectedBranch({
            value: voucher.hotelBranchId,
            label: voucher.hotelBranchName || "",
          });
        }
      } else {
        setSelectedBranch({
