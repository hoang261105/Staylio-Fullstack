import { Edit2, Eye, Power, Tag, Calendar, Percent, Coins, ShoppingCart } from "lucide-react";
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        ...
                        className={`p-1.5 rounded-lg transition-colors ${voucher.status === VoucherStatus.ACTIVE
                          ? "text-red-500 hover:bg-red-50"
                          : "text-green-500 hover:bg-green-50"
                          }`}
                        title={voucher.status === VoucherStatus.ACTIVE ? "Vô hiệu hóa" : "Kích hoạt"}
                      >
      ...
      {!isLoading && vouchers.length > 0 && (
        <div className="p-4 border-t border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between text-sm text-gray-500 gap-4 bg-gray-50/50">
          <div>
            Hiển thị <span className="font-medium text-gray-900">{vouchers.length}</span> trên tổng số{" "}
