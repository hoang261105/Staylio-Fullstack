...
export default function RoomFormUpdate({
  onClose,
  isOpen,
  room,
}: RoomFormUpdateProps) {
  ...
  const { mutateAsync: updateRoom, isPending } = useUpdateRoomMutation(
    room?.id ?? 0,
  );
  ...
  useEffect(() => {
    if (isOpen && room) {
      setFormData({
        ...
        floor: room.floor,
        utilityIds: room.utilities?.map((u: any) => u.id) || [],
        imageUrls: room.images?.map((img: any) => img.imageUrl) || [],
      });
      setImages(
      ...
      if (branches && branches.length > 0) {
        ...
        if (branch) {
          setSelectedBranch({
            value: branch.id,
            label: branch.hotelBranchName,
          });
        } else {
          setSelectedBranch({
            value: room.hotelBranchId,
            label: room.hotelBranchName,
          });
        }
      } else {
        setSelectedBranch({
  ...
  const handleUtilitiesChange = (options: readonly SelectOption[]) => {
    setFormData((prev) => ({
      ...prev,
      utilityIds: options.map((o) => o.value),
    }));
  ...
  const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Array.from(files).forEach(async (file, index) => {
      ...
      try {
        ...
        setImages((prev) =>
          prev.map((img) =>
            img.id === imgId ? { ...img, url, isUploading: false } : img,
          ),
        );
      } catch (error) {
    ...
    >
      <div className="bg-card text-foreground rounded-xl shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col animate-in fade-in zoom-in duration-200">
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto flex-1 p-6">
          ...
          <div className="mb-4">
            ...
            <Select<SelectOption, true>
              ...
              value={utilityOptions.filter((opt) =>
                formData.utilityIds?.includes(opt.value),
              )}
          ...
          <div className="mb-4">
            <label className="block text-sm mb-2">Thêm hình ảnh phòng</label>
            <input
              ref={fileInputRef}
              type="file"
              multiple
              ...
              className="hidden"
              id="room-update-image-upload"
            />
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {images.map((img, index) => (
                <div
                  key={img.id}
                  className="relative aspect-square rounded-lg overflow-hidden border border-border bg-muted flex items-center justify-center group"
                >
                  <img
                    src={img.preview}
                    alt="preview"
              ...
              <label
                htmlFor="room-update-image-upload"
              ...
              >
                ...
                <span className="text-sm text-muted-foreground">Thêm ảnh mới</span>
              </label>
            </div>
            {room?.images && room.images.length > 0 && (
              <p className="text-xs text-muted-foreground mt-2 italic">
                * Phòng đã có {room.images.length} ảnh. Bạn có thể thêm nhiều
                ảnh mới ở đây.
              </p>
            )}
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-border">
            ...
            >
              {(isPending || images.some((img) => img.isUploading)) && (
