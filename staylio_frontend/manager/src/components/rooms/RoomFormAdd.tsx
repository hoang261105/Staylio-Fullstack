...
export default function RoomFormAdd({
  ...
  type ImageState = { id: string; url: string; preview: string; isUploading: boolean; isError?: boolean; is360: boolean };
  ...
  const handleSubmit = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    clearAllErrors();
    try {
      const imageUrls = images.filter(img => !img.is360).map(img => img.url).filter(url => url !== "");
      const vr360Urls = images.filter(img => img.is360).map(img => img.url).filter(url => url !== "");
