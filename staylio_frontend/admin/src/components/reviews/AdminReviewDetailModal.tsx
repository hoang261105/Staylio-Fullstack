...
import { X, Building2, Star, MessageSquareQuote, Info, Reply, Calendar, ShieldCheck, ShieldAlert } from "lucide-react";
import { useReviewById, useUpdateReviewStatusMutation } from "@common/hooks/useReviews";
...
export default function AdminReviewDetailModal({ reviewId, onClose }: AdminReviewDetailModalProps) {
  const { data: review, isLoading, isError } = useReviewById(reviewId);
  const { mutate: updateStatus, isPending } = useUpdateReviewStatusMutation(reviewId);
  
  const [confirmAction, setConfirmAction] = useState<{isOpen: boolean, action: ReviewStatus | null}>({isOpen: false, action: null});

  const handleConfirm = () => {
    if (!confirmAction.action) return;
    updateStatus(confirmAction.action, {
      onSuccess: () => {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
                ...
                <div className="p-4">
                  {review.replyComment ? (
                    <div className="space-y-2">
                      <p className="text-gray-800 text-sm whitespace-pre-wrap leading-relaxed bg-white p-3 rounded-lg border border-blue-100">
                        {review.replyComment}
                      </p>
                    ...
                    <p className="text-sm italic text-gray-500">Khách sạn chưa phản hồi đánh giá này.</p>
                  )}
                </div>
              </div>
        ...
        <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex items-center justify-end gap-3">
          {!isLoading && review && review.status === ReviewStatus.PENDING && (
            <>
              <button
                onClick={() => setConfirmAction({ isOpen: true, action: ReviewStatus.HIDDEN })}
                className="px-5 py-2.5 text-sm font-medium text-red-600 bg-red-50 border border-red-200 rounded-xl hover:bg-red-100 transition-colors shadow-sm flex items-center gap-1.5"
              >
                <ShieldAlert className="w-4 h-4" />
                Từ chối (Ẩn)
              </button>
              <button
                onClick={() => setConfirmAction({ isOpen: true, action: ReviewStatus.VISIBLE })}
                className="px-5 py-2.5 text-sm font-medium text-white bg-emerald-600 border border-emerald-600 rounded-xl hover:bg-emerald-700 transition-colors shadow-sm flex items-center gap-1.5"
              >
          ...
          {!isLoading && review && review.status === ReviewStatus.VISIBLE && (
            <button
                onClick={() => setConfirmAction({ isOpen: true, action: ReviewStatus.HIDDEN })}
                className="px-5 py-2.5 text-sm font-medium text-red-600 bg-red-50 border border-red-200 rounded-xl hover:bg-red-100 transition-colors shadow-sm flex items-center gap-1.5"
              >
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 flex flex-col gap-4 animate-in zoom-in-95 duration-200">
            <div className="flex items-center gap-3 text-lg font-bold text-gray-900">
              {confirmAction.action === ReviewStatus.VISIBLE ? (
            ...
            <p className="text-gray-600 text-sm">
              Bạn có chắc chắn muốn <strong className={confirmAction.action === ReviewStatus.VISIBLE ? "text-emerald-600" : "text-red-600"}>{confirmAction.action === ReviewStatus.VISIBLE ? "duyệt hiển thị" : "ẩn"}</strong> đánh giá này không?
            </p>
            <div className="flex justify-end gap-3 mt-2">
              <button
                onClick={() => setConfirmAction({ isOpen: false, action: null })}
                disabled={isPending}
              ...
              <button
                ...
                className={`px-4 py-2 text-sm font-medium text-white rounded-xl transition-colors flex items-center gap-2 ${
                  confirmAction.action === ReviewStatus.VISIBLE ? "bg-emerald-600 hover:bg-emerald-700 shadow-sm shadow-emerald-600/20" : "bg-red-600 hover:bg-red-700 shadow-sm shadow-red-600/20"
                } disabled:opacity-50`}
              >
