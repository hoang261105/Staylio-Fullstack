...
interface AdminVoucherApproveModalProps {
  voucher: VoucherResponse | null;
  onClose: () => void;
}

export default function AdminVoucherApproveModal({
  voucher,
  onClose,
}: AdminVoucherApproveModalProps) {
  if (!voucher) return null;

  const { mutate, isPending } = useUpdateApprovalStatusVoucherMutation(voucher.id);

  const handleConfirm = () => {
    mutate(ApprovalStatus.CONFIRMED, {
      onSuccess: () => {
        onClose();
      },
    });
  ...
  return (
    ...
    >
      ...
      >
        <div className="px-6 py-5 flex items-center justify-between border-b border-emerald-100 bg-emerald-50">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-emerald-100 text-emerald-600">
              <CheckCircle className="w-5 h-5" />
            </div>
            <div>
              <h2 className="font-bold text-gray-900 text-base">Duyệt Voucher</h2>
              <p className="text-xs text-gray-500 mt-0.5">Xác nhận phê duyệt voucher này</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-6 space-y-5">
          <div className="flex items-start gap-3 p-4 rounded-xl border bg-emerald-50 border-emerald-200 text-emerald-800">
            ...
            <p className="text-sm leading-relaxed">
              Bạn có chắc chắn muốn duyệt voucher này? Sau khi duyệt, voucher sẽ có thể được kích hoạt để sử dụng.
            </p>
          </div>
        ...
        <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex items-center justify-end gap-3">
          ...
          <button
            ...
            disabled={isPending}
            className="px-5 py-2.5 text-sm font-semibold text-white rounded-xl transition-all flex items-center gap-2 shadow-sm disabled:opacity-60 disabled:cursor-not-allowed bg-emerald-500 hover:bg-emerald-600 shadow-emerald-500/25"
          >
            {isPending ? (
              <>
